...
const MaintainTables = () => {
  const { tables, refetch } = useItemsContext();
  ...
  const handleAddTable = async () => {
    setLoading(true);
    postApiRequest(`/api/v2/table/create`)
      .then((res) => {
        if (res) {
          refetch();
          setLoading(false);
  ...
  const handleSubmit = (e) => {
    ...
    if (data) {
      setEditLoading(true);
      patchApiRequest(`/api/v2/table/edit/${tableData?._id}`, data)
        .then((res) => {
          if (res) {
            refetch();
  ...
  const handleTableDelete = async (item) => {
    ...
    try {
      const res = await deleteApiRequest(`/api/v2/table/delete/${item._id}`);
      if (res) {
        refetch();
